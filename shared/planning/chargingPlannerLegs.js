import { buildObstacleAwareRoute } from "./zonePlannerRouting.js";
import { dist } from "./zonePlannerGeometry.js";
import { estimateRouteEnergy } from "./energyModel.js";
import { estimateTurnEnergy } from "./energyEstimator.js";
import {
  EPS,
  copyPoint,
  ...
  polylineLength,
  samePoint,
} from "./chargingPlannerGeometry.js";
import { dominates } from "./chargingPlannerFrontier.js";
...
export const findLegFrontier = ({
  start, end, startFuel, stations, polygons, batteryRange, surfaceZones, energyOptions, segmentCache, incomingPoint,
}) => {
  ...
  enqueueLabel({
    ...
    incomingPoint,
    heading: incomingPoint ? Math.atan2(start.y - incomingPoint.y, start.x - incomingPoint.x).toFixed(8) : null,
  ...
  while (open.length) {
    ...
    if (!frontiers[label.node].includes(labelIndex) || label.node === 1) continue;
    for (const edge of adjacency[label.node]) {
      const transitionEnergy = edge.energy + estimateTurnEnergy(label.incomingPoint, label.point, edge.route[1], { surfaceZones, ...energyOptions });
      if (transitionEnergy > label.fuel + FUEL_EPS) continue;
      const rawFuel = label.fuel - transitionEnergy;
      ...
      enqueueLabel({
        node: edge.to,
        point: stationPoint,
        cost: label.cost + edge.distance + transitionEnergy * ENERGY_SCORE_WEIGHT + stationDetourPenalty,
        distance: label.distance + edge.distance,
        energy: label.energy + transitionEnergy,
        incomingPoint: edge.route.length > 1 ? edge.route.at(-2) : label.incomingPoint,
        heading: edge.route.length > 1 ? Math.atan2(stationPoint.y - edge.route.at(-2).y, stationPoint.x - edge.route.at(-2).x).toFixed(8) : label.heading,
