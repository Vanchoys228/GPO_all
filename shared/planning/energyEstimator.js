import { DEFAULT_PAYLOAD_KG, DEFAULT_SPEED_MPS, SURFACE_PROFILES, SURFACE_ZONE_PRESETS, getSurfaceProfileByKey } from "./energyProfiles.js";
import { resolveSurfaceAtPoint, splitSegmentBySurfaces } from "./energySurfaceZones.js";
...
export const estimateTurnEnergy = (prev, current, next, {
  surfaceZones = SURFACE_ZONE_PRESETS, speedMps = DEFAULT_SPEED_MPS,
  payloadKg = DEFAULT_PAYLOAD_KG, includeTurnPenalty = true,
} = {}) => {
  if (!includeTurnPenalty || !prev || !next || dist(prev, current) <= EPS || dist(current, next) <= EPS) return 0;
  const angle = Math.abs(normalizeAngle(Math.atan2(next.y - current.y, next.x - current.x) - Math.atan2(current.y - prev.y, current.x - prev.x)));
  const { profile } = resolveSurfaceAtPoint(current, surfaceZones);
  return angle * profile.turnEnergyPerRad * calcSpeedFactor(profile, speedMps) * calcPayloadFactor(profile, payloadKg);
};
...
export const estimateRouteEnergy = (route, {
  ...
  for (let index = 1; index < route.length; index += 1) {
    ...
    for (const { fraction, profile } of splitSegmentBySurfaces(from, to, surfaceZones)) {
      totalEnergy += segmentDistance * fraction * profile.energyPerMeter * calcSpeedFactor(profile, speedMps) * calcPayloadFactor(profile, payloadKg);
      weightedSlipRisk += profile.slipRisk * segmentDistance * fraction;
  ...
  if (includeTurnPenalty) {
    for (let index = 1; index < route.length - 1; index += 1) {
      ...
      totalEnergy += estimateTurnEnergy(prev, current, next, { surfaceZones, speedMps, payloadKg });
