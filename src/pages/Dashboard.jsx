...
export default function Dashboard() {
  ...
  const [leftSidebarCollapsed, setLeftSidebarCollapsed] = useState(
    () => loadPlannerUiState().leftCollapsed
  );
  const [rightSidebarCollapsed, setRightSidebarCollapsed] = useState(
    () => loadPlannerUiState().rightCollapsed
  );

  const collapseLeftSidebar = useCallback(() => {
    setLeftSidebarCollapsed(true);
    const s = loadPlannerUiState();
    savePlannerUiState({ ...s, leftCollapsed: true });
  }, []);

  const expandLeftSidebar = useCallback(() => {
    setLeftSidebarCollapsed(false);
    const s = loadPlannerUiState();
    savePlannerUiState({ ...s, leftCollapsed: false });
  }, []);

  const collapseRightSidebar = useCallback(() => {
    setRightSidebarCollapsed(true);
    const s = loadPlannerUiState();
    savePlannerUiState({ ...s, rightCollapsed: true });
  }, []);

  const expandRightSidebar = useCallback(() => {
    setRightSidebarCollapsed(false);
    const s = loadPlannerUiState();
    savePlannerUiState({ ...s, rightCollapsed: false });
  }, []);
  ...
  const removeZone = (zoneId) => {
    ...
    if (activeLimitZoneId === zoneId) {
      setActiveLimitZoneId(nextZones[0]?.id ?? null);
    }
    if (nextZones.length === 0) {
      setActivePointKind((kind) => (kind === "limit" ? "visit" : kind));
    }
    clearRouteState({ dropSolvedRoute: false });
    setStatus(
      nextZones.length === 0
        ? "Все ограничивающие зоны удалены. При необходимости нажмите «Новая зона»."
        : "Ограничивающая зона удалена."
    );
  };
  ...
  const addPointFromCanvas = (event) => {
    ...
    if (activePointKind === "limit") {
      if (!activeLimitZoneId) {
        setStatus("Сначала создайте зону и выберите её в списке справа.");
        return;
      }
      if (plannerModel.activeZone?.closed) {
        setStatus(
  ...
  return (
    <div className="flex h-screen min-h-0 bg-stone-100 text-stone-900">
      {leftSidebarCollapsed ? (
        <SidebarCollapseRail side="left" onExpand={expandLeftSidebar} />
      ) : (
        <PlannerLeftSidebar
      ...
      {rightSidebarCollapsed ? (
        <SidebarCollapseRail side="right" onExpand={expandRightSidebar} />
      ) : (
        <PlannerRightSidebar
